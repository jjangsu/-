...
def checkGameEnd(self, ori_y, ori_x):
    # hori
    if self.checkStone(ori_x, ori_y, -1, 0) + self.checkStone(ori_x, ori_y, 1, 0) >= 3:
        return True

    # vert
    if self.checkStone(ori_x, ori_y, 0, -1) + self.checkStone(ori_x, ori_y, 0, 1) >= 3:
        return True

    # backslash
    if self.checkStone(ori_x, ori_y, -1, -1) + self.checkStone(ori_x, ori_y, 1, 1) >= 3:
        return True
